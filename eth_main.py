            ...
            query GetDexTrades($first: Int, $token0: String!, $token1: String!, $date: ISO8601DateTime!) {
                ethereum(network: ethereum) {
                    dexTrades(
                        ...
                        date: {since: $date, not: "2023-12-04T00:00:00Z", notIn: "2023-12-01T14:00:00Z"}
                    ) {
                        timeInterval {  
...
@eth_uniswap.get("/tokensInfo")
async def tokensInfo(token0: str = "", token1: str = ""):
    ethPrice = float(requests.get("https://api.binance.com/api/v3/ticker/price?symbol=ETHUSDT").json()['price'])
    token0 = token0.lower()
    token1 = token1.lower()
    ...
    response = requests.post('https://api.thegraph.com/subgraphs/name/uniswap/uniswap-v3',
